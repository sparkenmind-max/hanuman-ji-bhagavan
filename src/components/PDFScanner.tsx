...
import { Upload, Eye, Database, Zap, BookOpen, Calendar, X, ToggleLeft, ToggleRight, Circle, CheckCircle, Hash, CreditCard as Edit3, Key } from 'lucide-react';
import { supabase } from '../lib/supabase';
import { convertPdfToImages, performExtraction, ExtractedQuestion, setGeminiApiKeys } from '../lib/gemini';
...
export function PDFScanner() {
  ...
  const scanAndExtractQuestions = async () => {
    ...
    try {
      ...
      toast.success(`✅ Using ${validApiKeys.length} API key${validApiKeys.length > 1 ? 's' : ''} for scanning`);
    } catch (error) {
  ...
  const saveAllToDatabase = async () => {
    ...
    // Get the first valid year from pdfUploads
    const validPdfs = getValidPdfs();
    if (validPdfs.length === 0) {
      toast.error('Please provide year information for the questions');
      return;
    }

    // Use the year from the first PDF upload
    const defaultYear = validPdfs[0].year;
    ...
    try {
      ...
      // Save all questions with the default year
      const data = await savePdfQuestions(extractedQuestions, defaultYear, selectedSlot, selectedPart, enabledQuestionTypes);
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 mb-8">
          ...
          <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
            ...
            <ul className="text-sm text-blue-800 space-y-1">
              ...
              <li>• Perfect for scanning multiple PDFs without hitting rate limits</li>
            </ul>
          </div>
          ...
          {validApiKeys.length === 0 && (
            <div className="mt-4 p-3 bg-red-50 border border-red-200 rounded-lg">
              <p className="text-red-800 text-sm font-medium">
                ❌ Please provide at least one Gemini API key to start scanning
              </p>
            </div>
          )}
        </div>
        ...
        <div className="flex gap-4 justify-center mb-8">
          <button
            ...
            disabled={validPdfs.length === 0 || isScanning || !selectedExam || !selectedCourse || enabledQuestionTypes.length === 0 || validApiKeys.length === 0}
